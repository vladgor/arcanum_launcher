...
Procedure openWnd(hidden.b)
  If OpenWindow(#wnd, 374, 199, 360, 340, "ArcanumLauncher " + lVersion,  #PB_Window_SystemMenu | #PB_Window_MinimizeGadget | #PB_Window_TitleBar | #PB_Window_ScreenCentered | #PB_Window_Invisible)
    ...
    ButtonGadget(#btnGo, 250, 300, 100, 25, "Launch") 
    ButtonGadget(#btnGetMod, 10, 300, 125, 25, "Checking connection...")
...
Procedure openDescriptionWnd(mod.s)
  If OpenWindow(#wndDescription, 374, 199, 650, 550, mod + " — description, ArcanumLauncher " + lVersion,  #PB_Window_SystemMenu | #PB_Window_MinimizeGadget | #PB_Window_MaximizeGadget | #PB_Window_SizeGadget | #PB_Window_TitleBar | #PB_Window_ScreenCentered, WindowID(#wnd))
...
Procedure fillGetModsList()
  text.s = ReadHTTPFile("https://dl.dropboxusercontent.com/u/100848097/arcanumMods/mods.zip")
...
Procedure downloadAndUnzipMod(*value)
  ...
  SetGadgetText(#lblDownloading, "Preparing to download " + mod + "...")
  ...
  If UrlToFileWithProgress(tempName, mods()\link, mods()\size)
    ...
    If mods()\size = FileSize(tempName)
      SetGadgetText(#lblDownloading, "Unzipping " + mod + "...")
      ...
      If PureZIP_ExtractFiles(tempName,"*.*", pathToUnzip,#True) <> #Null
        d("Success!")
        SetGadgetText(#lblDownloading, "Installing of " + mod + " was successfully finished")
      ...
      Else ;if cant unzip
        d("!ERROR: can't unzip")
        d("Aborted!")
        SetGadgetText(#lblDownloading, "ERROR: Can't unzip " + mod)
      EndIf
  ...
  Else ;if cant receive file
    ...
    d("Aborted!")
    SetGadgetText(#lblDownloading, "ERROR: Can't download " + mod)
  EndIf
...
;---TODO: backup and unbackup windows, updating script
...
Procedure tUpdateMod(*val)
  ...
  SetGadgetText(#lblDownloading, "Backuping mod...")
  d("Backuping mod...")
  ...
  If backupMod(mod, FormatDate("(%dd.%mm) before updating to " + mods()\newversion, Date()), FormatDate("%dd/%mm/%yyyy, %hh:%ii", Date()) + Chr(13) + Chr(10) + LSet("",10, "-") + Chr(13) + Chr(10) + "Updating to new version (from " + mods()\version + " to " + mods()\newversion + ")")
    ...
    SetGadgetText(#lblDownloading, "Preparing to download update...")
    d("Downloading...")
    
    If UrlToFileWithProgress(tempName, mods()\link, mods()\size)
      SetGadgetText(#lblDownloading, "Deleting old files...")
      d("Downloaded!")
      
      d("Deleting old mod's folder...")
      
      If DeleteDirectory("mods\" + mod, "*.*", #PB_FileSystem_Force | #PB_FileSystem_Recursive)
        SetGadgetText(#lblDownloading, "Unzipping new files...")
        d("Deleted!")
        ...
        If PureZIP_ExtractFiles(tempName,"*.*", pathToUnzip, #True) <> #Null
          d("Unzipped!")
          SetGadgetText(#lblDownloading, "Unbackuping old saves...")
          
          d("Backuping old saves...")
          
          If unbackupMod(mod, FormatDate("(%dd.%mm) before updating to " + mods()\newversion, Date()))
            SetGadgetText(#lblDownloading, "Mod has been successfully updated")
            d("Backuped!")
          ...
          Else
            ...
            errorText = "Can't unbackup mod"
          EndIf
        Else ;if not extracted
          d("Can't unzip mod")
          errorText = "Can't unzip mod"
        EndIf
      Else ;if not deleted
        d("Can't delete mod's directory")
        errorText = "Can't delete mod's directory"
      EndIf
      
    Else ;if not downloaded
      d("Can't download mod")
      errorText = "Can't download mod"
    EndIf
  Else ;if not backuped
    d("Can't backup mod's files")
    errorText = "Can't backup mod's files"
  EndIf
  
  SetGadgetText(#lblDownloading, errorText + ". Closing window...")
  Delay(2000)
...
Procedure tBackupMod(*val)
 ...
 SetGadgetText(#lblMakeBackupStatus, "Backuping saves...")
 
 If result
   SetGadgetColor(#lblMakeBackupStatus, #PB_Gadget_FrontColor, RGB(104,163,0))
   SetGadgetText(#lblMakeBackupStatus, "Everything is okay. Closing window...")
 ...
 ElseIf result = #L_BACKUPMOD_CANTCREATEFILE
   ...
   SetGadgetText(#lblMakeBackupStatus, "Backup with this name already exists!")
   Delay(1500)
...
Procedure tUnbackupMod(*val)
  ...
  If result
    ...
    SetGadgetText(#lblUnbackupModStatus, "Everything is okay. Closing window...")
  ...
  Else
    ...
    If result = #L_UNBACKUPMOD_CANTUNZIP
      SetGadgetText(#lblUnbackupModStatus, "Can't unzip files")
    Else
      SetGadgetText(#lblUnbackupModStatus, "Can't unbackup. Unspecified error")
    EndIf
...
;   If checkConnection("arcanummods.net23.net", 80)
;     SetGadgetText(#btnGetMod, "Get more mods")
;     DisableGadget(#btnGetMod, 0)
;   Else
;     SetGadgetText(#btnGetMod, "Can't connect")
;   EndIf

  If readHttpFile("https://dl.dropboxusercontent.com/u/100848097/arcanumMods/test.txt") <> ""
    SetGadgetText(#btnGetMod, "Get more mods")
    DisableGadget(#btnGetMod, 0)
  Else
    SetGadgetText(#btnGetMod, "Can't connect")
...
Procedure checkModForUpdates(mod.s)
  ...
  If t
    message.s = "There is new version available (your version: " + version + ", new version: " + newVersion + ")" + Chr(13)
    message = message + whatsnew + Chr(13)
    message = message + "Do you want to update this mod? Your game progress will be saved (I hope)."
...
If (argMod = "")
  ...
  If CreatePopupMenu(#menu)
    ...
    MenuItem(#menuCreateShortcut, "Create shortcut")
    MenuBar()
    MenuItem(#menuBackupMod, "Backup mod")
    MenuItem(#menuUnbackupMod, "Unbackup mod")
    MenuBar()
  ...
  If Event = #PB_Event_Gadget
    Select EventGadget
      ...
      Case 22 To 37 ;если один из ключей для ярлыков
        ...
        Else
          MessageRequester("Error :(", "This argument is already written", #MB_ICONERROR)
        EndIf
...
; EnableCompileCount = 551
; EnableBuildCount = 214