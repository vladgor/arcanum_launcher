...
; Ч launcher now can store old debug-files in launcher\logs\ directory 
;   (watch for new "storeLogs" config-line (1 - on, 0 - off))
; Ч added launcher-specific -lwindowed command-line argument
; Ч launcherDebug.txt is launcher\debug.txt from now
; Ч launcherConfig.cfg is launcher.cfg from now

; TODO Ч what's new при обновлении лончера
;      Ч не сохран€ет ключи дл€ мода, если нет конфигурационного файла
...
If cfgCheckForUpdate = 1 
  ...
  If newVersion <> "0"
    message.s = ReplaceString(ReplaceString(localMsgLauncherNewVersionFound, "%version", lVersion), "%newversion", newVersion)
    answer.i = MessageRequester("", message, #PB_MessageRequester_YesNo | #MB_ICONINFORMATION)
    
    If answer = #PB_MessageRequester_Yes
      If updateLauncher()
        End
      Else
        MessageRequester("", localMsgLauncherUpdateFailed, #MB_ICONERROR)
      EndIf
    EndIf
  EndIf
EndIf
...
;not completely done
Procedure activateFix(name.s)
  ;TODO: applying different fixes
  
  dIndent(3)
  d("activating fix: " + name)
  d("---")
  path.s = "launcher\fixes\" + name + "\"
  
  If RenameFile(path + "arcanum-w.exe", "arcanum-w.exe")
    d("moved: " + path + "arcanum-w.exe" + " -> " + "arcanum-w.exe")
  Else
    d("ERROR: can't rename " + path + "arcanum-w.exe" + " -> " + "arcanum-w.exe")
  EndIf
  
  If RenameFile(path + "ddraw.dll", "ddraw.dll")
    d("moved: " + path + "ddraw.dll" + " -> " + "ddraw.dll")
  Else
    d("ERROR: can't rename " + path + "ddraw.dll" + " -> " + "ddraw.dll")
  EndIf
  
  If RenameFile(path + "aqrit.cfg", "aqrit.cfg")
    d("moved: " + path + "aqrit.cfg" + " -> " + "aqrit.cfg")
  Else
    d("ERROR: can't rename " + path + "aqrit.cfg" + " -> " + "aqrit.cfg")
  EndIf
  
  ;old.s = ""
  ;new.s = ""
  
  ;SearchFilesInit(path,"*.*",1)
  ;Repeat
  ;  old = SearchFilesGet()
  ;  If old = "" : Break : EndIf
  ;  new = RemoveString(old, path)
  ;  RenameFile(old, new)
  ;ForEver
...
;not completely done
Procedure deactivateFix(name.s)
  dIndent(3)
  d("deactivating fix: " + name)
  d("---")
  path.s = "launcher\fixes\" + name + "\"
  
  
  If RenameFile("arcanum-w.exe", path + "arcanum-w.exe")
    d("moved: " + "arcanum-w.exe" + " -> " + path + "arcanum-w.exe")
  Else
    d("ERROR: can't rename " + "arcanum-w.exe" + " -> " + path + "arcanum-w.exe")
  EndIf
  
  If RenameFile("ddraw.dll", path + "ddraw.dll")
    d("moved: " + "ddraw.dll" + " -> " + path + "ddraw.dll")
  Else
    d("ERROR: can't rename " + "ddraw.dll" + " -> " + path + "ddraw.dll")
  EndIf
  
  If RenameFile("aqrit.cfg", path + "aqrit.cfg")
    d("moved: " + "aqrit.cfg" + " -> " + path + "aqrit.cfg")
  Else
    d("ERROR: can't rename " + "aqrit.cfg" + " -> " + path + "aqrit.cfg")
  EndIf
EndProcedure
...
Procedure runArcanum(modToActivate.s)
  ...
  If errors > 0
   If cfgLanguage = ""
     ...
     MessageRequester("", Str(errors) + temp + "occured. See " + Chr(34) + "launcher\debug.txt" + Chr(34) + " for details", #MB_ICONERROR)
   Else
...
;{
  ...
  ;if -lwindowed was found
  If FindString(arg, "-lwindowed")
    activateFix("windowed")
    windowed = #True
    arg = ReplaceString(arg, "-lwindowed", "")
  EndIf
  ...
  Else
    Program = RunProgram("Arcanum-w.exe", arg, "", #PB_Program_Open)
  EndIf
...
Repeat 
  ...
  If Event = #PB_Event_Gadget
    Select EventGadget
      ...
      Case 25 To 42 ;if command line arguments
        ...
        If FindString(text2, text, 0) = 0
          ...
          If EventGadget = #lblShortcutWindowed ; -no3d is needed for windowed mode
            If FindString(text2, "-no3d", 0) = 0
              SetGadgetText(#txtShortcut, GetGadgetText(#txtShortcut) + " -no3d ")
            EndIf
          EndIf
...
; Executable = C:\Dropbox\јрканум\arcanum launcher\0.6.0.0\release\launcher.exe
