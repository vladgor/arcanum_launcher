...
;+ оптимизировал загрузку
;+ считает количество ошибок при активации и деактивации мода
;+ проверяет, не зупущено ли несколько лончеров в одной папке
;+ проверяет, есть ли соединение с интернетом
;
;* arcanumN.dat и arcanum.patchN теперь не переименовываются
;* папка /maps теперь удаляется, а не переносится
...
Procedure downloadAndUnzipMod(*mod)
  ...
  SetGadgetText(#lblDownloading, "Preparing to download " + dMod + "...")
...
Procedure openWndGetMod(modName.s)
  If OpenWindow(#wndGetMod, 222, 231, 764, 480, "Mod Downloader, ArcanumLauncher " + lVersion,  #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_Invisible | #PB_Window_TitleBar, WindowID(#wnd))
    SmartWindowRefresh(#wndGetMod, 1) 
...
Procedure updateMod(name.s, link.s)
...
Procedure checkModForUpdates(mod.s)
  ...
  ;TODO: починить для пользовательских модов
...
Repeat 
    ...
    If Event = #PB_Event_Gadget
      Select EventGadget
        ...
        Case 20 To 35 ;если один из ключей для ярлыков
...
; EnableCompileCount = 330
; EnableBuildCount = 166