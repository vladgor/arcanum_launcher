...
;TOFIX: после нескольких открытий перестаёт появляться сообщение "У вас нет бэкапов для этого мода"
;       исправить! (запилить очистку списка с бэкапами)
;TOFIX: не скачивается и не показывается описание первого мода в серверном списке, если установлен мод ниже


;TODO : комбинация модов (временная и перманентная)
;TODO : не показывать список с модами для скачивания, если он пуст
...
;{ GetMod's section
  ...
  Procedure fillGetModsList()
    ...
    While i <= count
      ...
      ElseIf key <> ""
        If skip = #False
          mods()\size = mods()\size + key + Chr(13)
        EndIf
      EndIf
...
; EnableCompileCount = 663
; EnableBuildCount = 262