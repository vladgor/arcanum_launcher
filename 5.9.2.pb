...
; благодарности 
;      http://kriss80858.deviantart.com/art/Arcanum-logo-1-395035946 — за лого для фона
;      http://www.terra-arcanum.com/phpBB/viewtopic.php?t=15208      — за описание ключей запуска 
 
;version 6
; относительно стабильна

;TOFIX: (исправлено?) после нескольких открытий перестаёт появляться сообщение "У вас нет бэкапов для этого мода"
;       исправить! (запилить очистку списка с бэкапами)
;TOFIX: (исправлено?) не скачивается и не показывается описание первого мода в серверном списке, если установлен мод ниже
...
Procedure.i activateMod(name.s)
  ...
  ;смотрим, как папки надо перемещать и чистить
  text.s = getFileContent(modFolder + "modSettings.txt")
  If text <> ""
  ...
  If ListSize(mods()\DirsToReplace()) = 0
    ;если не указаны папки для реплэйса
    ;то заносим в список data\players\ и папки с сейвами (\modules\modules\save\)
    AddElement(mods()\DirsToReplace())
    mods()\DirsToReplace() = "data\Players"
    
    If ExamineDirectory(0, modFolder + "modules\", "*.*") ; копируем сейвы сингловых персонажей
      While NextDirectoryEntry(0)
        If DirectoryEntryType(0) = #PB_DirectoryEntry_Directory
          If beginsWith(DirectoryEntryName(0), ".") = 0
            saveFolderName.s = "modules\" + DirectoryEntryName(0) + "\save"
  ...
  Repeat
    ...
    ForEach mods()\DirsToReplace()
      If (beginsWith(temp, mods()\DirsToReplace())) ;если файл лежит в любой из папок dirsToReplace
...
;{ GetMod's section
  Procedure getImagesToCache(text.s)
    ...
    While f <> 0
      ...
      found.b = #False
      ForEach cached() ;если уже в списке кэшированных файлов
        If cached()\http = url
...
Repeat 
  ...
  If Event = #PB_Event_Gadget
    Select EventGadget
      ...
      Case 23 To 38 ;если один из ключей для ярлыков
...
; EnableCompileCount = 745
; EnableBuildCount = 287